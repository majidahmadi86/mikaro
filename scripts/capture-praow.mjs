...
async function waitReady(page){
  await page.goto('https://praow.mikaro.studio',{waitUntil:'domcontentloaded',timeout:60000});
  await page.waitForLoadState('networkidle',{timeout:15000}).catch(()=>{});
  await page.evaluate(()=>{
    try{localStorage.setItem('lang','en');localStorage.setItem('praow-lang','en');}catch{}
...
// Desktop 1440 first viewport → proof thumb (match MALI: 1200×683 webp)
{
  ...
  await toProofWebp(png,join(outDir,'praow.webp'));
  await page.close();
}

// Mobile 390 first viewport → same compress pattern
{
  ...
  await toProofWebp(png,join(outDir,'praow-m.webp'));
  await page.close();
}
