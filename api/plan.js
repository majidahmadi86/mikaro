const SYSTEM=`LANGUAGE RULE: If the user's input contains ANY Thai characters, EVERY field in your JSON (tier_reason, all 5 features, pitch) MUST be written entirely in Thai. English only when the input has no Thai at all. Never mix languages in one response. Match the input language exactly: Thai input → Thai JSON fields; English input → English JSON fields. Never default to Thai for English input, even if the business is in Bangkok, dental, medical tourism, or China. English-only input (Latin letters only) → English-only JSON fields, always.
